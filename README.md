...
&emsp;&emsp;**Target→Settings→Mode: Connect under reset**设置复位模式下可以连接已烧录bin固件的板子，按下reset点击connect，松开后连接成功，点击**Full chip erase**可用于擦除固件。或者采用另外一种方式将其恢复正常的开发板，就需要`boot0`跳线帽接3.3V上电，进行下载正常开发板程序，还原跳线帽就可以了

&emsp;&emsp;注意：U盘拖拽下载功能有所限制，一种固件只能对应一系列单片机。烧录的是 stm321103xb_if 接口固件，因此，只支持 stm321103xb 系列单片机拖拽下载。如果你想要拖拽下载 STM32F401re 系列单片机程序，则必须将固件更新为 STM32F401re_if 的接口固件
